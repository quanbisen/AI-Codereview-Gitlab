...
class WeComNotifier:
    ...
    def _get_webhook_url(self, project_name=None, url_slug=None, gitlab_group=None):
        ...
        if not project_name and not url_slug and not gitlab_group:
            if fallback_url:
                return fallback_url
            raise ValueError("未提供项目名称、url_slug、gitlab组，且未设置默认的企业微信 Webhook URL。")

        # 构建环境变量键名和优先级列表
        env_keys = []
        if project_name:
            env_keys.append((f"WECOM_WEBHOOK_URL_{project_name.upper()}", f"项目 '{project_name}'"))
        if url_slug:
            env_keys.append((f"WECOM_WEBHOOK_URL_{url_slug.upper()}", f"URL Slug '{url_slug}'"))
        if gitlab_group:
            env_keys.append((f"WECOM_WEBHOOK_URL_{gitlab_group.upper()}", f"GitLab Group '{gitlab_group}'"))
        ...
        raise ValueError(f"未找到{error_sources}对应的企业微信 Webhook URL，且未设置默认的 Webhook URL。")
